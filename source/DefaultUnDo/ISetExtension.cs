...
namespace DefaultUnDo
{
    ...
    public static class ISetExtension
    {
        ...
        private class UnDoSet<T> : ICollectionExtension.UnDoCollection<T>, ISet<T>
        {
            ...
            public UnDoSet(IUnDoManager manager, ISet<T> source)
                : base(manager, source)
            {
                _source = source;
            }
            ...
            bool ISet<T>.Add(T item) => _manager.DoAdd(_source, item);
        ...
        public static ISet<T> AsUnDo<T>(this ISet<T> source, IUnDoManager manager) => new UnDoSet<T>(
